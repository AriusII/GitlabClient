...
public sealed class TopicsRepositoryTests
{
    ...
    public async Task ListAsync_BuildsTopicsRoute_AndDeserializesTheTopic()
    {
        ...
        TopicsRepository repository = new(new GitLabApiConnection(httpClient));
    ...
    [Fact]
    public async Task CreateAsync_PostsTheTopicBody_OmittingUnsetFields()
    {
        ...
        Assert.Equal("https://gitlab.example/api/v4/topics", handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("""{"name":"gitlab","title":"GitLab","organization_id":1}""", sentBody);
        Assert.Equal(1, topic.Id);
    }

    [Fact]
    public async Task UpdateAsync_PutsOnlyTheFieldsThatWereSet()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        Assert.Equal("https://gitlab.example/api/v4/topics/1", handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("""{"description":"Everything GitLab"}""", sentBody);
    }
