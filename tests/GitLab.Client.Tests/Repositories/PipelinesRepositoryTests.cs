...
public sealed class PipelinesRepositoryTests
{
    [Fact]
    public async Task GetAsync_BuildsPipelineRoute_AndDeserializesPipeline()
    {
        ...
        PipelinesRepository repository = new(connection);
    ...
    public async Task CreateAsync_PostsToSingularPipelineRoute_WithSerializedBody_AndDeserializesCreatedPipeline()
    {
        ...
        CreatePipelineRequest request = new() { Ref = "develop" };
    ...
    [Fact]
    public async Task ListTriggerJobsAsync_BuildsTheTriggerJobsRoute_AndDeserializesTheDownstreamPipeline()
    {
        const string Json = """
                            [
                              {
        ...
        TriggerJobListOptions options = new() { Scope = ["success", "failed"], PerPage = 20 };

        List<GitLabJob> jobs = [];
        await foreach (GitLabJob job in repository.ListTriggerJobsAsync(1, 47, options,
                           TestContext.Current.CancellationToken))
        {
            jobs.Add(job);
        }
