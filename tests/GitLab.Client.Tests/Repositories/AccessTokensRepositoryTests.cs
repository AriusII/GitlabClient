...
public sealed class AccessTokensRepositoryTests
{
    ...
    public async Task ListForProjectAsync_BuildsProjectRoute_WithQueryOptions_AndDeserializesTokens()
    {
        ...
        AccessTokensRepository repository = new(connection);
        ...
        Assert.Equal(new DateTimeOffset(2025, 3, 1, 0, 0, 0, TimeSpan.Zero), token.ExpiresAt);
    ...
    public async Task RotateForProjectAsync_PostsExpiresAt_ToTheRotateRoute()
    {
        ...
        Assert.Equal(new DateTimeOffset(2026, 6, 30, 0, 0, 0, TimeSpan.Zero), token.ExpiresAt);
    }
