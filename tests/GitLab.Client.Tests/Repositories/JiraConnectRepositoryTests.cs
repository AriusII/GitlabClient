...
public sealed class JiraConnectRepositoryTests
{
    ...
    private static JiraConnectRepository CreateRepository(HttpMessageHandler handler, out HttpClient httpClient)
    {
        httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        return new JiraConnectRepository(new GitLabApiConnection(httpClient));
    }
    ...
    public async Task SubscribeNamespaceAsync_PostsToJiraConnectRoute_AndSendsJwtAndNamespacePath()
    {
        ...
        JiraConnectRepository repository = CreateRepository(handler, out HttpClient httpClient);
        using (httpClient)
        {
