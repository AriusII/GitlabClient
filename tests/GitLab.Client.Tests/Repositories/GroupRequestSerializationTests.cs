...
/// <summary>
///     Pins the wire names <see cref="GitLabJsonContext" /> resolves for the two large group request
...
public sealed class GroupRequestSerializationTests
{
    ...
    [
        ...
        "duo_template_project_id", "allow_personal_snippets", "built_in_project_templates_enabled",
        "lock_built_in_project_templates_enabled", "create_code_review_flow_consent"
    ];

    /// <summary>
    ///     The binary <c>avatar</c> field (sent through <c>SetAvatarAsync</c> instead), the deprecated
    ///     <c>emails_disabled</c>, and the three schema-less object fields the spec declares with no
    ///     properties at all (<c>ai_settings_attributes</c>, <c>foundational_agents_statuses</c>,
    ///     <c>duo_namespace_access_rules</c>) are deliberately absent from both records.
    /// </summary>
