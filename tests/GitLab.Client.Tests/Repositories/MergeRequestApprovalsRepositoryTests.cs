...
public sealed class MergeRequestApprovalsRepositoryTests
{
    ...
    public async Task GetApprovalsAsync_BuildsApprovalsRoute_AndDeserializesApprovers()
    {
        ...
        MergeRequestApprovalsRepository repository = CreateRepository(handler, out HttpClient httpClient);
        using (httpClient)
        {
    ...
    private static MergeRequestApprovalsRepository CreateRepository(StubHttpMessageHandler handler,
        out HttpClient httpClient)
    {
        httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        return new MergeRequestApprovalsRepository(new GitLabApiConnection(httpClient));
    }
}