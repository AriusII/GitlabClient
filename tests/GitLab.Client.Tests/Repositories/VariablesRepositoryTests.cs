...
public sealed class VariablesRepositoryTests
{
    ...
    public async Task ListProjectVariablesAsync_BuildsVariablesRoute_WithPaging_AndDeserializesVariables()
    {
        ...
        VariablesRepository repository = new(connection);

        List<GitLabVariable> variables = new();
        await foreach (GitLabVariable item in repository.ListProjectVariablesAsync(1,
                           new VariableListOptions { PerPage = 50 }, TestContext.Current.CancellationToken))
        {
            variables.Add(item);
        }
    ...
    public async Task ListGroupVariablesAsync_BuildsGroupVariablesRoute_AndEncodesNamespacedGroupPath()
    {
        ...
        Assert.Equal("TEST_VARIABLE_1", Assert.Single(variables).Key);
    }

    [Fact]
    public async Task GetGroupVariableAsync_EscapesKey_AndDeserializesVariable()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(VariableJson, Encoding.UTF8, "application/json")
        ...
        GitLabVariable variable =
            await repository.GetGroupVariableAsync(9, "SCOPED/KEY", TestContext.Current.CancellationToken);
    ...
    public async Task CreateGroupVariableAsync_PostsToGroupVariablesRoute_WithSerializedBody()
    {
        ...
        CreateVariableRequest request = new()
        {
            Key = "TEST_VARIABLE_1", Value = "TEST_1", Masked = true, EnvironmentScope = "production"
        };
    ...
    [Fact]
    public async Task UpdateGroupVariableAsync_PutsToGroupVariablesRoute()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        GitLabVariable variable = await repository.UpdateGroupVariableAsync(9, "TEST_VARIABLE_1",
            new UpdateVariableRequest { Value = "TEST_2", Raw = false }, TestContext.Current.CancellationToken);
    ...
    public async Task DeleteGroupVariableAsync_SendsDeleteToGroupVariablesRoute()
    {
        ...
        await repository.DeleteGroupVariableAsync(9, "TEST_VARIABLE_1", TestContext.Current.CancellationToken);
    ...
    public async Task UpdateInstanceVariableAsync_PutsToTheAdminRoute()
    {
        ...
        UpdateVariableRequest request = new() { Value = "rotated", Protected = true };
        ...
        Assert.Equal("https://gitlab.example/api/v4/admin/ci/variables/TEST_VARIABLE_1",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("""{"value":"rotated","protected":true}""", sentBody);
    }
