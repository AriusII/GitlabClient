...
public sealed class DeployKeysRepositoryTests
{
    ...
    [Fact]
    public async Task CreateAsync_PostsToInstanceWideRoute_WithSerializedBody_AndDeserializesProjectAccess()
    {
        string json = $$"""
                        {
                          "id": 21,
        ...
        // The instance-wide request has no can_push - it is never sent, even though the shared DTO carries it.
        Assert.DoesNotContain("can_push", sentBody, StringComparison.Ordinal);

        Assert.Equal(21, key.Id);
        Assert.Null(key.CanPush);
        Assert.Equal(7, key.ProjectsWithWriteAccess?.Id);
        Assert.Equal("acme/shop", key.ProjectsWithWriteAccess?.PathWithNamespace);
        Assert.Equal(8, key.ProjectsWithReadonlyAccess?.Id);
        Assert.Equal("acme/docs", key.ProjectsWithReadonlyAccess?.PathWithNamespace);
    }
