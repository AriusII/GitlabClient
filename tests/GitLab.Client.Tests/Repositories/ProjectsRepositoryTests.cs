...
public sealed class ProjectsRepositoryTests
{
    ...
    ///     mis-cased C# name (which GitLab would answer 200 to, and silently ignore) out of the library.
    /// </summary>
    [Fact]
    public async Task CreateAsync_PostsToProjects_AndSpellsEveryFieldTheWayGitLabDoes()
    {
        ...
        Assert.Equal(
            """
            {"name":"GitLab","path":"gitlab","issues_access_level":"enabled","pages_access_level":"public",
            """ +
            """
            "container_expiration_policy_attributes":{"cadence":"1d","enabled":true,"keep_n":10},
            """ +
            """
            "visibility":"internal","topics":["devops","ruby"],"merge_method":"rebase_merge",
            """ +
            """
            "namespace_id":42,"import_url":"https://example.com/repo.git"}
            """,
            harness.SentBody);
    }

    [Fact]
    public async Task CreateForUserAsync_PostsToTheAdministratorRoute()
    {
        using Harness harness = new(HttpStatusCode.Created, ProjectJson);
        ...
        Assert.Equal("""{"name":"GitLab"}""", harness.SentBody);
    }

    [Fact]
    public async Task UpdateAsync_SendsOnlyTheFieldsThatWereSet()
    {
        using Harness harness = new(HttpStatusCode.OK, ProjectJson);
        ...
        Assert.Equal(
            """
            {"description":"Now with docs","auto_devops_enabled":true,"mr_default_target_self":false,
            """ +
            """
            "spp_repository_pipeline_access":true,"ci_pipeline_variables_minimum_override_role":"maintainer",
            """ +
            """
            "ci_id_token_sub_claim_components":["project_path","ref_type"],
            """ +
            """
            "merge_train_enforcement":"enforce_for_all_users","reviewer_assignment_strategy":"code_owners"}
            """,
            harness.SentBody);
    }
    ...
    [Fact]
    public async Task ForkAsync_WithNoRequest_StillPostsAnObject()
    {
        using Harness harness = new(HttpStatusCode.Created, ProjectJson);

        await harness.Repository.ForkAsync("gitlab-org/gitlab", cancellationToken: TestContext.Current
            .CancellationToken);

        Assert.Equal(HttpMethod.Post, harness.Method);
        Assert.Equal("https://gitlab.example/api/v4/projects/gitlab-org%2Fgitlab/fork", harness.RequestUri);
        Assert.Equal("{}", harness.SentBody);
    }
    ...
    /// <summary>
    ///     Wires a <see cref="ProjectsRepository" /> to a stubbed transport, capturing the one request it
    ...
    private sealed class Harness : IDisposable
    {
        ...
        public Harness(HttpStatusCode status, string? json)
        {
            ...
            Repository = new ProjectsRepository(new GitLabApiConnection(_httpClient));
        }

        public ProjectsRepository Repository { get; }
