...
public sealed class ExternalStatusChecksRepositoryTests
{
    ...
    public async Task ListAsync_BuildsExternalStatusChecksRoute_AndDeserializesChecks()
    {
        ...
        ExternalStatusChecksRepository repository = CreateRepository(handler, out HttpClient httpClient);
        using (httpClient)
        {
    ...
    private static ExternalStatusChecksRepository CreateRepository(StubHttpMessageHandler handler,
        out HttpClient httpClient)
    {
        httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        return new ExternalStatusChecksRepository(new GitLabApiConnection(httpClient));
    }
}