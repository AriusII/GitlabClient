...
public sealed class ApprovalRulesEndpointTests
{
    ...
    public async Task ListForProjectAsync_BuildsProjectApprovalRulesRoute_AndDeserializesRules()
    {
        ...
        ApprovalRulesClient repository = CreateRepository(handler, out HttpClient httpClient);
        using (httpClient)
        {
    ...
    [Fact]
    public async Task CreateGroupAsync_PostsOnlyTheGroupApprovalRuleSchema()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        using (httpClient)
        {
            CreateGroupApprovalRuleRequest request = new()
            {
                Name = "Group policy",
                ApprovalsRequired = 1,
                RuleType = GitLabApprovalRuleType.Regular,
                UserIds = [7],
                GroupIds = [90]
            };

            GitLabApprovalRule rule =
                await repository.CreateGroupAsync(55, request, TestContext.Current.CancellationToken);
            ...
            Assert.Contains("\"rule_type\":\"regular\"", sentBody, StringComparison.Ordinal);
            Assert.Contains("\"user_ids\":[7]", sentBody, StringComparison.Ordinal);
            ...
            Assert.True(body.RootElement.TryGetProperty("user_ids", out _));
            Assert.True(body.RootElement.TryGetProperty("group_ids", out _));
            Assert.Equal(21, rule.Id);
        }
    }

    [Fact]
    public async Task CreateForGroupAsync_LegacyProjectRequestProjectsOnlyGroupFields()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        using (httpClient)
        {
            ...
            using JsonDocument body = JsonDocument.Parse(sentBody!);
            Assert.Equal(5, body.RootElement.EnumerateObject().Count());
            Assert.False(body.RootElement.TryGetProperty("usernames", out _));
            Assert.False(body.RootElement.TryGetProperty("protected_branch_ids", out _));
            Assert.False(body.RootElement.TryGetProperty("applies_to_all_protected_branches", out _));
            Assert.False(body.RootElement.TryGetProperty("report_type", out _));
            Assert.False(body.RootElement.TryGetProperty("coverage_minimum_threshold", out _));
            Assert.False(body.RootElement.TryGetProperty("vulnerabilities_allowed", out _));
            Assert.False(body.RootElement.TryGetProperty("scanners", out _));
            Assert.False(body.RootElement.TryGetProperty("severity_levels", out _));
            Assert.False(body.RootElement.TryGetProperty("vulnerability_states", out _));
        }
    }

    [Fact]
    public async Task UpdateGroupAsync_PutsTheGroupRuleTypeToTheGroupRuleRoute()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        using (httpClient)
        {
            UpdateGroupApprovalRuleRequest request = new()
            {
                Name = "Group policy",
                ApprovalsRequired = 2,
                RuleType = GitLabApprovalRuleType.AnyApprover,
                UserIds = [7],
                GroupIds = [90]
            };

            GitLabApprovalRule rule =
                await repository.UpdateGroupAsync(55, 21, request, TestContext.Current.CancellationToken);
            ...
            Assert.Contains("\"name\":\"Group policy\"", sentBody, StringComparison.Ordinal);
            Assert.Contains("\"approvals_required\":2", sentBody, StringComparison.Ordinal);
            Assert.Contains("\"rule_type\":\"any_approver\"", sentBody, StringComparison.Ordinal);
            Assert.Contains("\"user_ids\":[7]", sentBody, StringComparison.Ordinal);
            Assert.Contains("\"group_ids\":[90]", sentBody, StringComparison.Ordinal);
            using JsonDocument body = JsonDocument.Parse(sentBody!);
            Assert.Equal(5, body.RootElement.EnumerateObject().Count());
    ...
    [Fact]
    public async Task UpdateForGroupAsync_LegacyProjectRequestProjectsOnlyGroupFields()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        using (httpClient)
        {
            ...
            Assert.False(body.RootElement.TryGetProperty("protected_branch_ids", out _));
            Assert.False(body.RootElement.TryGetProperty("applies_to_all_protected_branches", out _));
            Assert.False(body.RootElement.TryGetProperty("remove_hidden_groups", out _));
            Assert.False(body.RootElement.TryGetProperty("coverage_minimum_threshold", out _));
            Assert.False(body.RootElement.TryGetProperty("vulnerabilities_allowed", out _));
            Assert.False(body.RootElement.TryGetProperty("scanners", out _));
            Assert.False(body.RootElement.TryGetProperty("severity_levels", out _));
    ...
    private static ApprovalRulesClient CreateRepository(StubHttpMessageHandler handler, out HttpClient httpClient)
    {
        httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        return new ApprovalRulesClient(new GitLabApiConnection(httpClient));
    }
}