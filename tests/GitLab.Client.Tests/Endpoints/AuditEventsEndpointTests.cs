...
public sealed class AuditEventsEndpointTests
{
    ...
    public async Task ListAsync_BuildsAuditEventsRoute_WithQueryOptions_AndDeserializesTheDetailsPayload()
    {
        ...
        AuditEventsClient repository = new(connection);

        AuditEventListOptions options = new()
        {
            EntityType = GitLabAuditEventEntityType.Project,
            EntityId = 7,
    ...
    [Fact]
    public async Task ListForGroupAsync_WithoutOptions_SendsNoQueryString()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
        ...
        await foreach (GitLabAuditEvent _ in
                       repository.ListForGroupAsync(42, cancellationToken: TestContext.Current.CancellationToken))
        {
            Assert.Fail("The stub returns an empty page.");
        }

        Assert.Equal("https://gitlab.example/api/v4/groups/42/audit_events",
    ...
    [Fact]
    public async Task GetForGroupAsync_BuildsGroupAuditEventByIdRoute_AndEncodesNamespacedPath()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(AuditEventJson, Encoding.UTF8, "application/json")
        ...
        GitLabAuditEvent auditEvent = await repository.GetForGroupAsync("parent-group/subgroup", 4,
            TestContext.Current.CancellationToken);

        Assert.Equal(HttpMethod.Get, handler.LastRequest?.Method);
        Assert.Equal("https://gitlab.example/api/v4/groups/parent-group%2Fsubgroup/audit_events/4",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("project_archived", auditEvent.EventName);
    }

    [Fact]
    public async Task ListForProjectAsync_BuildsProjectAuditEventsRoute_WithDateWindow_AndDeserializesPayload()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent($"[{AuditEventJson}]", Encoding.UTF8, "application/json")
        ...
        List<GitLabAuditEvent> events = [];
        await foreach (GitLabAuditEvent item in repository.ListForProjectAsync("gitlab-org/gitlab",
        ...
        Assert.Equal(HttpMethod.Get, handler.LastRequest?.Method);
        Assert.Equal(
            "https://gitlab.example/api/v4/projects/gitlab-org%2Fgitlab/audit_events?created_after=2026-01-01T00:00:00Z&per_page=100",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("group/myproject", Assert.Single(events).Details!.Value.GetProperty("entity_path").GetString());
    }

    [Fact]
    public async Task GetForProjectAsync_BuildsProjectAuditEventByIdRoute()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent(AuditEventJson, Encoding.UTF8, "application/json")
        ...
        GitLabAuditEvent auditEvent = await repository.GetForProjectAsync(42, 4,
            TestContext.Current.CancellationToken);

        Assert.Equal(HttpMethod.Get, handler.LastRequest?.Method);
        Assert.Equal("https://gitlab.example/api/v4/projects/42/audit_events/4",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal(4, auditEvent.Id);
    }
}