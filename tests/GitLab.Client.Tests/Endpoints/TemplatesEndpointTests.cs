...
public sealed class TemplatesEndpointTests
{
    ...
    public async Task ListDockerfilesAsync_BuildsTemplatesRoute_AndDeserializesSummaries()
    {
        ...
        TemplatesClient repository = new(connection);
    ...
    [Fact]
    public async Task GetLicenseAsync_UsesTheLicenseKeyAndCarriesTheCopyrightSubstitutions()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
        ...
        Assert.Equal(
            "https://gitlab.example/api/v4/templates/licenses/apache-2.0"
