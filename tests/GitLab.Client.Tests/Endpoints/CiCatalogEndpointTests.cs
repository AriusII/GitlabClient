...
public sealed class CiCatalogEndpointTests
{
    [Fact]
    public async Task PublishAsync_PostsToTheCatalogPublishRoute_AndDeserializesTheCatalogUrl()
    {
        ...
        CiCatalogClient repository = new(connection);
    ...
    [Fact]
    public async Task PublishAsync_WithExplicitNullMetadata_PreservesTheRequiredNullableMember()
    {
        const string Json = """{ "catalog_url": "https://gitlab.example/explore/catalog/component" }""";
        ...
        await repository.PublishAsync(7, new CiCatalogPublishRequest { Metadata = null },
            TestContext.Current.CancellationToken);

        Assert.Equal("{\"metadata\":null}", sentBody);
    }
