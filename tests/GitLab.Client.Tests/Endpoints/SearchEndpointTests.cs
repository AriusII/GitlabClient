...
public sealed class SearchEndpointTests
{
    ...
    public async Task SearchProjectsAsync_HitsTheInstanceRoute_WithTheProjectsScope_AndDeserializesProjects()
    {
        ...
        SearchClient repository = new(connection);
    ...
    [Fact]
    public async Task SearchGroupsAsync_HitsTheInstanceRoute_WithTheGroupsScope()
    {
        const string Json = """
                            [{ "id": 19, "name": "Platform", "path": "platform", "full_path": "platform" }]
                            """;
        ...
        List<GitLabGroup> groups = await CollectAsync(repository.SearchGroupsAsync("platform",
            new SearchListOptions { IncludeArchived = true }, TestContext.Current.CancellationToken));

        Assert.Equal("https://gitlab.example/api/v4/search?search=platform&scope=groups&include_archived=true",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        GitLabGroup group = Assert.Single(groups);
        Assert.Equal(19, group.Id);
        Assert.Equal("platform", group.FullPath);
    }

    [Fact]
    public async Task SearchCommitsAsync_HitsTheInstanceRoute_WithTheCommitsScope()
    {
        const string Json = """
                            [{ "id": "aabbcc", "short_id": "aabbcc", "title": "Fix search", "web_url": "https://gitlab.example/group/project/-/commit/aabbcc" }]
                            """;
        ...
        List<GitLabCommit> commits = await CollectAsync(repository.SearchCommitsAsync("fix search", null,
            TestContext.Current.CancellationToken));

        Assert.Equal("https://gitlab.example/api/v4/search?search=fix%20search&scope=commits",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("aabbcc", Assert.Single(commits).ShortId);
    }

    [Fact]
    public async Task SearchNotesAsync_HitsTheInstanceRoute_WithTheNotesScope()
    {
        const string Json = """
                            [{ "id": 77, "body": "Search result" }]
                            """;
        ...
        List<GitLabNote> notes = await CollectAsync(repository.SearchNotesAsync("search result", null,
            TestContext.Current.CancellationToken));

        Assert.Equal("https://gitlab.example/api/v4/search?search=search%20result&scope=notes",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("Search result", Assert.Single(notes).Body);
    }

    [Fact]
    public async Task SearchMilestonesAsync_HitsTheInstanceRoute_WithTheMilestonesScope()
    {
        const string Json = """
                            [{ "id": 25, "iid": 4, "title": "19.0", "state": "active", "web_url": "https://gitlab.example/group/project/-/milestones/4" }]
                            """;
        ...
        List<GitLabMilestone> milestones = await CollectAsync(repository.SearchMilestonesAsync("19.0", null,
            TestContext.Current.CancellationToken));

        Assert.Equal("https://gitlab.example/api/v4/search?search=19.0&scope=milestones",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal(4, Assert.Single(milestones).Iid);
    }
    ...
    [Fact]
    public async Task SearchGroupGroupsAsync_EncodesTheGroupPath_AndUsesTheGroupsScope()
    {
        const string Json = """
                            [{ "id": 20, "name": "Child", "path": "child", "full_path": "platform/child" }]
                            """;
        ...
        List<GitLabGroup> groups = await CollectAsync(repository.SearchGroupGroupsAsync("platform/parent",
            "child", null, TestContext.Current.CancellationToken));

        Assert.Equal("https://gitlab.example/api/v4/groups/platform%2Fparent/search?search=child&scope=groups",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("platform/child", Assert.Single(groups).FullPath);
    }
