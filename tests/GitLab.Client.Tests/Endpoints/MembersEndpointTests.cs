...
public sealed class MembersEndpointTests
{
    /// <summary>
    ///     A group member as GitLab sends it, including group-scoped SAML/SCIM identity data and a custom
    ///     member role.
    /// </summary>
    ...
    public async Task ListAsync_BuildsMembersRoute_AndDeserializesEachMember()
    {
        ...
        MembersClient repository = new(connection);
    ...
    [Fact]
    public async Task RemoveAsync_WithOptions_ProjectsBothGitLabRemovalFlags()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.NoContent));
        ...
        RemoveProjectMemberOptions options = new() { SkipSubresources = true, UnassignIssuables = true };

        await repository.RemoveAsync(ProjectId.FromId(5), 1, options, TestContext.Current.CancellationToken);

        Assert.Equal(HttpMethod.Delete, handler.LastRequest?.Method);
        Assert.Equal(
            "https://gitlab.example/api/v4/projects/5/members/1?skip_subresources=true&unassign_issuables=true",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
    }
    ...
    [Fact]
    public async Task ListAsync_ProjectsEveryDirectProjectMemberFilter()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("[]", Encoding.UTF8, "application/json")
        ...
        ProjectMemberListOptions options = new()
        {
            Query = "smith",
            UserIds = [1, 2],
            SkipUsers = [3],
        ...
        await foreach (GitLabMember _ in repository.ListAsync(ProjectId.FromPath("group/project"), options,
                           TestContext.Current.CancellationToken))
        {
            // Draining the sequence is what issues the request; the route is asserted below.
        }
        ...
        string? requestUri = handler.LastRequest?.RequestUri?.AbsoluteUri;
        Assert.Equal(HttpMethod.Get, handler.LastRequest?.Method);
        Assert.StartsWith("https://gitlab.example/api/v4/projects/group%2Fproject/members?", requestUri,
    ...
    public async Task GetIncludingInheritedForGroupAsync_BuildsRoute_AndDeserializesTheFullMemberShape()
    {
        ...
        Assert.Equal("group_saml", member.GroupSamlIdentity?.Provider);
        Assert.Equal("4085", member.GroupSamlIdentity?.ExternUid);
        Assert.Equal(52, member.GroupSamlIdentity?.SamlProviderId);
        Assert.Equal("4085", member.GroupScimIdentity?.ExternUid);
        Assert.Equal(9, member.GroupScimIdentity?.GroupId);
        Assert.True(member.GroupScimIdentity?.Active);
        Assert.Equal(2, member.MemberRole?.Id);
        Assert.Equal("Custom role", member.MemberRole?.Name);
        Assert.Equal(30, member.MemberRole?.BaseAccessLevel);
    }
    ...
    public async Task ListBillableMembershipsForGroupAsync_BuildsMembershipsRoute_AndDeserializesTheAccessLevelObject()
    {
        ...
        GitLabBillableMembershipAccessLevel accessLevel = Assert.IsType<GitLabBillableMembershipAccessLevel>(
            only.AccessLevel);
        Assert.Equal(30, accessLevel.IntegerValue);
        Assert.Equal("Developer", accessLevel.StringValue);
        Assert.Equal("release-approver", accessLevel.CustomRole);
    }
