...
public sealed class MergeRequestsEndpointTests
{
    [Fact]
    public async Task GetAsync_BuildsProjectScopedRoute_AndDeserializesMergeRequest()
    {
        ...
        MergeRequestsClient repository = new(connection);
    ...
    public async Task CreateAsync_PostsRequestBody_AndDeserializesCreatedMergeRequest()
    {
        ...
        CreateMergeRequestRequest request = new()
        {
            ...
            Squash = true,
            MergeAfter = new DateTimeOffset(2026, 9, 11, 9, 30, 0, TimeSpan.Zero),
        ...
        Assert.Contains("\"target_project_id\":84", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"assignee_ids\":[5,6]", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"reviewer_ids\":[7]", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"labels\":[\"backend\",\"ready\"]", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"remove_source_branch\":true", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"allow_collaboration\":true", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"squash\":true", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"merge_after\":\"2026-09-11T09:30:00+00:00\"", sentBody, StringComparison.Ordinal);
        Assert.Contains("\"approvals_before_merge\":2", sentBody, StringComparison.Ordinal);
