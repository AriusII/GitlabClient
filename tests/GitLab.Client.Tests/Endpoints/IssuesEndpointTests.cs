...
public sealed class IssuesEndpointTests
{
    ...
    public async Task GetAsync_BuildsIssueRoute_AndDeserializesIssue()
    {
        ...
        IssuesClient repository = new(connection);
    ...
    public async Task GetAsync_DeserializesTheFullIssuePayload_IncludingItsNestedEntities()
    {
        string json = $$"""
                        {
                          ...
                          "epic": {
                            ...
                            "iid": "7",
                            "title": "Reliability",
                            "url": "https://gitlab.example/groups/gitlab-org/-/epics/7",
                            "group_id": "9970",
                            "human_readable_end_date": "Dec 31, 2026",
                            "human_readable_timestamp": "Jan 1, 2026"
                          },
        ...
        Assert.Equal("7", issue.EpicIid);
        Assert.Equal("42", issue.Epic?.Id);
        Assert.Equal("Reliability", issue.Epic?.Title);
        Assert.Equal(new Uri("https://gitlab.example/groups/gitlab-org/-/epics/7"), issue.Epic?.Url);
        Assert.Equal("9970", issue.Epic?.GroupId);
        Assert.Equal("Dec 31, 2026", issue.Epic?.HumanReadableEndDate);
    }
    ...
    public async Task CreateAsync_PostsToIssuesRoute_WithSerializedBody_AndDeserializesCreatedIssue()
    {
        ...
        Assert.Contains("\"epic_id\":42", sentBody, StringComparison.Ordinal);
    ...
    public async Task UpdateAsync_PutsToTheIssueRoute_WithOnlyTheSuppliedFields()
    {
        ...
        GitLabIssue issue = await repository.UpdateAsync(
            "gitlab-org/gitlab",
            6,
            new UpdateIssueRequest { Title = "Renamed", AddLabels = BugLabel, Confidential = true, EpicId = 42 },
            TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task DeleteMetricImageAsync_SendsDeleteToTheMetricImageRoute_AndDeserializesTheDeletedImage()
    {
        using StubHttpMessageHandler handler = Responds(MetricImageJson);
        using HttpClient httpClient = new(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        GitLabApiConnection connection = new(httpClient);
        ...
        GitLabMetricImage metricImage = await repository.DeleteMetricImageAsync(42, 6, 23,
            TestContext.Current.CancellationToken);

        Assert.Equal(HttpMethod.Delete, handler.LastRequest?.Method);
        Assert.Equal("https://gitlab.example/api/v4/projects/42/issues/6/metric_images/23",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal(23, metricImage.Id);
        Assert.Equal("cpu.png", metricImage.Filename);
        Assert.Equal(new Uri("https://grafana.example/d/abc"), metricImage.Url);
        Assert.Equal("CPU saturation", metricImage.Caption);
    }
}