...
public sealed class DeploymentsEndpointTests
{
    ...
    public async Task ListAsync_BuildsDeploymentsRoute_WithQueryOptions_AndDeserializesEachDeployment()
    {
        ...
        DeploymentsClient repository = new(connection);
    ...
    public async Task GetAsync_BuildsSingleDeploymentRoute_AndDeserializesTheExtendedShape()
    {
                                ...
                                "rules": [
                                  {
                                    ...
                                    "deployment_approvals": [
                                      {
                                        "status": "approved",
                                        "comment": "Approved through the protected-environment rule."
                                      }
                                    ]
                                  }
                                ]
        ...
        GitLabDeploymentApproval approval = Assert.Single(
            Assert.IsAssignableFrom<IReadOnlyList<GitLabDeploymentApproval>>(deployment.Approvals));
        Assert.Equal("approved", approval.Status);
        Assert.Equal("security-lead", approval.User?.Username);
        Assert.Equal("Looks safe.", approval.Comment);

        GitLabDeploymentApprovalSummary approvalSummary = Assert.IsType<GitLabDeploymentApprovalSummary>(
            deployment.ApprovalSummary);
        GitLabDeploymentApprovalRule approvalRule = Assert.Single(
            Assert.IsAssignableFrom<IReadOnlyList<GitLabDeploymentApprovalRule>>(approvalSummary.Rules));
        Assert.Equal(17, approvalRule.Id);
        Assert.Equal(100, approvalRule.UserId);
        Assert.Equal(23, approvalRule.GroupId);
        Assert.Equal(40, approvalRule.AccessLevel);
        Assert.Equal("Maintainer", approvalRule.AccessLevelDescription);
        Assert.Equal(2, approvalRule.RequiredApprovals);
        Assert.Equal(1, approvalRule.GroupInheritanceType);
        GitLabDeploymentApproval ruleApproval = Assert.Single(
            Assert.IsAssignableFrom<IReadOnlyList<GitLabDeploymentApproval>>(approvalRule.DeploymentApprovals));
        Assert.Equal("approved", ruleApproval.Status);
        Assert.Equal("Approved through the protected-environment rule.", ruleApproval.Comment);
    }
    ...
    [Fact]
    public async Task GetAsync_DeserializesSchemaPermittedPayloadWithoutIdentityOrStatus()
    {
        const string Json = """{ "approval_summary": { "rules": [] } }""";
        ...
        Assert.Null(deployment.Id);
        Assert.Null(deployment.Status);
        Assert.Empty(Assert.IsAssignableFrom<IReadOnlyList<GitLabDeploymentApprovalRule>>(
