...
public sealed class PackagesNuGetEndpointTests
{
    ...
    public async Task GetServiceIndexForGroupAsync_BuildsTheDashRoute_AndDeserializesResources()
    {
        ...
        PackagesNuGetClient repository = new(connection);
        ...
        GitLabNugetServiceResource resource = Assert.Single(index.Resources!);
        Assert.Equal(new Uri("https://gitlab.example.com/api/v4/projects/1/packages/nuget/query"), resource.AtId);
        Assert.True(resource.AtType is { ValueKind: JsonValueKind.String });
        Assert.Equal("SearchQueryService", resource.AtType!.Value.GetString());
        Assert.Equal("Filter and search for packages by keyword.", resource.Comment);
    }

    [Fact]
    public async Task GetServiceIndexAsync_PreservesTheProtocolMultiTypeResourceShape()
    {
        const string MultiTypeServiceIndexJson = """
                                                 {
                                                   "version": "3.0.0",
                                                   "resources": [
                                                     {
                                                       "@id": "https://gitlab.example.com/api/v4/projects/1/packages/nuget/query",
                                                       "@type": ["SearchQueryService", "SearchQueryService/3.5.0"]
                                                     }
                                                   ]
                                                 }
                                                 """;
        ...
        (HttpClient httpClient, StubHttpMessageHandler handler) = CreateClient(_ => new HttpResponseMessage(
            HttpStatusCode.OK)
        {
            Content = new StringContent(MultiTypeServiceIndexJson, Encoding.UTF8, "application/json")
        });
        ...
        JsonElement type = Assert.Single(index.Resources!).AtType!.Value;
        Assert.Equal(JsonValueKind.Array, type.ValueKind);

        JsonElement.ArrayEnumerator types = type.EnumerateArray();
        Assert.True(types.MoveNext());
        Assert.Equal("SearchQueryService", types.Current.GetString());
        Assert.True(types.MoveNext());
        Assert.Equal("SearchQueryService/3.5.0", types.Current.GetString());
