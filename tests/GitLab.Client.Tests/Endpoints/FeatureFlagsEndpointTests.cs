...
public sealed class FeatureFlagsEndpointTests
{
    ...
    public async Task ListAsync_BuildsFeatureFlagsRoute_AndDeserializesStrategiesAndScopes()
    {
        ...
        FeatureFlagsClient repository = new(connection);
    ...
    [Fact]
    public async Task ListUserListsAsync_BuildsTheUserListRoute_WithFilters_AndDeserializesPaths()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
        ...
        await foreach (GitLabFeatureFlagUserList list in repository.ListUserListsAsync(1,
                           new FeatureFlagUserListOptions { Search = "my list", Page = 2, PerPage = 50 },
                           TestContext.Current.CancellationToken))
        {
            lists.Add(list);
        }
        ...
        Assert.Equal(
            "https://gitlab.example/api/v4/projects/1/feature_flags_user_lists?search=my%20list&page=2&per_page=50",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
