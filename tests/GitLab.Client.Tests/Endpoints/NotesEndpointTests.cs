...
public sealed class NotesEndpointTests
{
    [Fact]
    ...
    [Fact]
    public async Task GetMergeRequestNoteAsync_DeserializesDiffPositionAndEmbeddedSuggestions()
    {
        const string Json = """
                            {
                              "id": 502,
                              "body": "Use the safer overload.",
                              "position": {
                                "base_sha": "a1b2c3d4",
                                "start_sha": "b2c3d4e5",
                                "head_sha": "c3d4e5f6",
                                "position_type": "text",
                                "new_path": "src/Client.cs",
                                "new_line": 42
                              },
                              "suggestions": [
                                {
        ...
        GitLabNote note = await repository.GetMergeRequestNoteAsync(1, 12, 502,
            TestContext.Current.CancellationToken);

        Assert.Equal("https://gitlab.example/api/v4/projects/1/merge_requests/12/notes/502",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.NotNull(note.Position);
        Assert.Equal(GitLabNotePositionType.Text, note.Position.PositionType);
        Assert.Equal("src/Client.cs", note.Position.NewPath);
        Assert.Equal(42, note.Position.NewLine);

        Assert.NotNull(note.Suggestions);
        GitLabSuggestion suggestion = Assert.Single(note.Suggestions);
        Assert.Equal(73, suggestion.Id);
        Assert.True(suggestion.Appliable);
        Assert.False(suggestion.Applied);
        Assert.Equal("Safe()", suggestion.ToContent);
    }
