...
public sealed class EventsEndpointTests
{
    ...
    public async Task ListAsync_BuildsInstanceEventsRoute_WithEveryFilter_AndDeserializesPushEvent()
    {
        ...
        EventsClient repository = new(connection);
        ...
        Assert.Equal(
            ...
            + "?scope=all&action=pushed&target_type=issue&before=2024-06-30&after=2024-01-01&sort=asc&page=3&per_page=50",
    ...
    [Fact]
    public async Task ListForUserAsync_EncodesAUsername_AndSupportsTheFirstPageOption()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("[]", Encoding.UTF8, "application/json")
        ...
        await foreach (GitLabEvent _ in repository.ListForUserAsync("ada lovelace", new EventListOptions { Page = 2 },
                           TestContext.Current.CancellationToken))
        {
            Assert.Fail("The stub returns an empty page.");
        }

        Assert.Equal("https://gitlab.example/api/v4/users/ada%20lovelace/events?page=2",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
    }
