...
public sealed class RemoteMirrorsEndpointTests
{
    [Fact]
    public void CreateRemoteMirrorRequest_AuthMethod_UsesGitLabWireValues()
    {
        CreateRemoteMirrorRequest publicKeyRequest = new()
        {
            Url = "ssh://git@example.com/group/project.git", AuthMethod = GitLabRemoteMirrorAuthMethod.SshPublicKey
        };
        CreateRemoteMirrorRequest passwordRequest = new()
        {
            Url = "https://user:password@example.com/group/project.git",
            AuthMethod = GitLabRemoteMirrorAuthMethod.Password
        };

        string publicKeyJson = JsonSerializer.Serialize(publicKeyRequest,
            GitLabJsonContext.Default.CreateRemoteMirrorRequest);
        string passwordJson = JsonSerializer.Serialize(passwordRequest,
            GitLabJsonContext.Default.CreateRemoteMirrorRequest);

        Assert.Contains("\"auth_method\":\"ssh_public_key\"", publicKeyJson, StringComparison.Ordinal);
        Assert.Contains("\"auth_method\":\"password\"", passwordJson, StringComparison.Ordinal);
    }
    ...
    public async Task ListAsync_BuildsRemoteMirrorsRoute_AndDeserializesEachMirror()
    {
        ...
        RemoteMirrorsClient repository = new(connection);
    ...
    public async Task CreateAsync_PostsToRemoteMirrorsRoute_WithSerializedBody_AndDeserializesCreatedMirror()
    {
        ...
        CreateRemoteMirrorRequest request = new()
        {
            ...
            Enabled = true,
            AuthMethod = GitLabRemoteMirrorAuthMethod.SshPublicKey,
    ...
    [Fact]
    public async Task GetPublicKeyAsync_HitsThePublicKeyRoute_AndDeserializesTheTypedResponse()
    {
        ...
        GitLabRemoteMirrorPublicKey publicKey =
        ...
        Assert.Equal("https://gitlab.example/api/v4/projects/42/remote_mirrors/101486/public_key",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("ssh-rsa AAAAB3NzaC1yc2EAAAADAQAB", publicKey.PublicKey);
    }

    [Fact]
    public async Task GetPublicKeyAsync_WithAnInvalidPayload_ThrowsJsonException()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("{ \"key\": \"missing expected member\" }", Encoding.UTF8,
                "application/json")
        });
        ...
        JsonException exception = await Assert.ThrowsAsync<JsonException>(() =>
            repository.GetPublicKeyAsync(42, 101486, TestContext.Current.CancellationToken));

        Assert.Equal("GitLab returned an invalid remote-mirror public-key response.", exception.Message);
    }
