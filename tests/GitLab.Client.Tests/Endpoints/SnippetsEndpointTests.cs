...
public sealed class SnippetsEndpointTests
{
    private const string SnippetJson = """
                                       {
                                         ...
                                         "expires_at": "2012-07-28T11:52:04+02:00",
                                         "project_id": null,
    ...
    private static SnippetsClient CreateRepository(HttpMessageHandler handler, out HttpClient httpClient)
    {
        httpClient = new HttpClient(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        return new SnippetsClient(new GitLabApiConnection(httpClient));
    }
    ...
    public async Task ListAsync_BuildsTheSnippetsRoute_AndDeserializesTheFullPayload()
    {
        ...
        SnippetsClient repository = CreateRepository(handler, out HttpClient httpClient);
        using (httpClient)
        {
            ...
            Assert.Equal(new DateTimeOffset(2012, 7, 28, 11, 52, 4, TimeSpan.FromHours(2)), only.ExpiresAt);
    ...
    [Fact]
    public async Task ListForProjectAsync_ProjectsTheSupportedPerPageOption()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("[]", Encoding.UTF8, "application/json")
        ...
        using (httpClient)
        {
            await foreach (GitLabSnippet _ in repository.ListForProjectAsync(7,
                               new ProjectSnippetListOptions { PerPage = 50 },
                               TestContext.Current.CancellationToken))
            {
                // Draining the sequence is what issues the request.
            }

            Assert.Equal("https://gitlab.example/api/v4/projects/7/snippets?per_page=50",
                handler.LastRequest?.RequestUri?.AbsoluteUri);
        }
    }
