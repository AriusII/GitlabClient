...
public sealed class RunnerControllersEndpointTests
{
    ...
    public async Task ListAsync_BuildsRunnerControllersRoute_AndDeserializesTheSummaryShape()
    {
        ...
        RunnerControllersClient repository = new(new GitLabApiConnection(httpClient));
    ...
    [Fact]
    public async Task GetScopesAsync_DeserializesTheOfficialScopeEnvelope()
    {
        const string Json = """
                            {
        ...
        GitLabRunnerControllerScopes scopes = await repository.GetScopesAsync(7,
            TestContext.Current.CancellationToken);
        ...
        GitLabRunnerControllerScope instanceScope = Assert.Single(
            Assert.IsAssignableFrom<IReadOnlyList<GitLabRunnerControllerScope>>(scopes.InstanceLevelScopings));
        GitLabRunnerControllerScope runnerScope = Assert.Single(
            Assert.IsAssignableFrom<IReadOnlyList<GitLabRunnerControllerScope>>(scopes.RunnerLevelScopings));
        Assert.Null(instanceScope.RunnerId);
        Assert.Equal(8, runnerScope.RunnerId);
        Assert.Equal(new DateTimeOffset(2025, 1, 16, 9, 0, 0, TimeSpan.Zero), runnerScope.CreatedAt);
    }

    [Fact]
    public async Task ListScopesAsync_FlattensTheOfficialScopeEnvelopeInStableOrder()
    {
        const string Json = """
                            {
                              "instance_level_scopings": [{ "created_at": "2025-01-15T09:00:00.000Z" }],
                              "runner_level_scopings": [{ "runner_id": 8, "created_at": "2025-01-16T09:00:00.000Z" }]
                            }
                            """;
        ...
        List<GitLabRunnerControllerScope> scopes = [];
        ...
        Assert.Equal([null, 8], scopes.Select(static scope => scope.RunnerId));
    }
    ...
    [Fact]
    public async Task CreateTokenAsync_PostsTheDescription_AndReturnsTheOneTimeSecret()
    {
        string? sentBody = null;
        using StubHttpMessageHandler handler = new(request =>
        {
        ...
        GitLabRunnerControllerTokenWithSecret token = await repository.CreateTokenAsync(7,
        ...
        Assert.Equal("glrct-created-secret", token.Token);
    }
