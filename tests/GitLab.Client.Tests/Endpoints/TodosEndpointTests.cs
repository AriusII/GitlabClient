...
public sealed class TodosEndpointTests
{
    ...
    public async Task ListAsync_BuildsRootTodosRoute_WithQueryOptions_AndDeserializesTodos()
    {
        ...
        TodosClient repository = new(connection);
        ...
        Assert.Contains("page=4", requestUri, StringComparison.Ordinal);
        ...
        Assert.True(todo.Target.HasValue);
        Assert.Equal(7, todo.Target.Value.GetProperty("id").GetInt64());
        Assert.Equal("Tighten API contracts", todo.Target.Value.GetProperty("title").GetString());
