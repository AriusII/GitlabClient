...
public sealed class UsersEndpointTests
{
    ...
    public async Task GetAsync_RequestsTheNumericUserRoute_AndDeserializesTheResponse()
    {
        ...
        UsersClient repository = new(connection);
    ...
    [Fact]
    public async Task GetAsync_WithOptions_RequestsTheCustomAttributeProjection()
    {
        const string json = """
                            {
                              "id": 7,
        ...
        using HttpClient httpClient = new(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        UsersClient repository = new(new GitLabApiConnection(httpClient));

        GitLabUser user = await repository.GetAsync(7, new UserGetOptions { WithCustomAttributes = true },
            TestContext.Current.CancellationToken);

        Assert.Equal("https://gitlab.example/api/v4/users/7?with_custom_attributes=true",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("cost_center", Assert.Single(user.CustomAttributes!).Key);
    }
    ...
    [Fact]
    public async Task CreateAsync_PostsMultipartFormFields_AndOmitsUnsetMembers()
    {
        ...
        Assert.Contains("name=email", body, StringComparison.Ordinal);
        Assert.Contains("octo@example.com", body, StringComparison.Ordinal);
        Assert.Contains("name=username", body, StringComparison.Ordinal);
        Assert.Contains("octocat", body, StringComparison.Ordinal);
        Assert.Contains("name=reset_password", body, StringComparison.Ordinal);
        Assert.Contains("name=skip_confirmation", body, StringComparison.Ordinal);
        Assert.Contains("name=group_id_for_saml", body, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("name=bio", body, StringComparison.Ordinal);

        Assert.Equal(7, created.Id);
    }

    [Fact]
    public async Task CreateAsync_WithAvatar_PostsMultipartFormFieldsAlongsideTheAvatar()
    {
        string? sentBody = null;
        string? contentType = null;
        using StubHttpMessageHandler handler = new(request =>
        ...
        using HttpClient httpClient = new(handler) { BaseAddress = new Uri("https://gitlab.example/api/v4/") };
        UsersClient repository = new(new GitLabApiConnection(httpClient));
        using MemoryStream avatarBytes = new("PNG-BYTES"u8.ToArray());

        GitLabUser created = await repository.CreateAsync(
            new CreateUserRequest
            {
                Email = "octo@example.com", Name = "Octo Cat", Username = "octocat", ResetPassword = true
            },
            new GitLabFileUpload { Content = avatarBytes, FileName = "avatar.png", FieldName = "unexpected" },
            TestContext.Current.CancellationToken);
        ...
        Assert.Contains("filename=avatar.png", body, StringComparison.Ordinal);
        Assert.DoesNotContain("name=unexpected", body, StringComparison.Ordinal);
        Assert.True(avatarBytes.CanRead);
        Assert.Equal(7, created.Id);
    }

    [Fact]
    public async Task UpdateAsync_PutsMultipartFormFields_ToTheNumericUserRoute()
    {
        string? sentBody = null;
        string? contentType = null;
        using StubHttpMessageHandler handler = new(request =>
        ...
        Assert.NotNull(sentBody);
        string body = sentBody.Replace("\"", string.Empty, StringComparison.Ordinal);
        Assert.Contains("name=organization", body, StringComparison.Ordinal);
        Assert.Contains("name=admin", body, StringComparison.Ordinal);
        Assert.DoesNotContain("name=username", body, StringComparison.Ordinal);

        Assert.Equal("Contractor until June.", updated.Note);
    }

    [Fact]
    public async Task UpdateAsync_WithAvatar_PutsMultipartFormFieldsAlongsideTheAvatar()
    {
        string? sentBody = null;
        string? contentType = null;
        using StubHttpMessageHandler handler = new(request =>
        ...
        GitLabUser updated = await repository.UpdateAsync(
            7,
            new UpdateUserRequest { Note = "Updated note", External = true },
            new GitLabFileUpload { Content = avatarBytes, FileName = "avatar.png", FieldName = "unexpected" },
            TestContext.Current.CancellationToken);
        ...
        Assert.NotNull(sentBody);
        string body = sentBody.Replace("\"", string.Empty, StringComparison.Ordinal);
        Assert.Contains("name=note", body, StringComparison.Ordinal);
        Assert.Contains("Updated note", body, StringComparison.Ordinal);
        Assert.Contains("name=external", body, StringComparison.Ordinal);
        Assert.Contains("name=avatar", body, StringComparison.Ordinal);
        Assert.Contains("filename=avatar.png", body, StringComparison.Ordinal);
        Assert.DoesNotContain("name=unexpected", body, StringComparison.Ordinal);
        Assert.True(avatarBytes.CanRead);
    ...
    public async Task ListFollowersAsync_AndListFollowingAsync_HitTheirOwnRoutes()
    {
        ...
        UsersClient followersRepository = new(followersConnection);
        ...
        UsersClient followingRepository = new(followingConnection);
    ...
    public async Task ListEnterpriseUsersAsync_BuildsTheGroupScopedRoute_WithTheProvidedOptions()
    {
                            ...
                            [
                              {
                                ...
                                "email": "octo@example.com",
                                "custom_attributes": [{ "key": "cost_center", "value": "42" }],
                                "scim_identities": [{ "active": true, "extern_uid": "abc", "group_id": "42" }]
                              }
                            ]
                            """;
        ...
        GitLabScimIdentity identity = Assert.Single(user2.ScimIdentities!);
        Assert.Equal("abc", identity.ExternUid);
