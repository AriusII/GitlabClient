...
public sealed class AccessRequestsEndpointTests
{
    private const string AccessRequestJson = """
                                             {
                                               ...
                                               "custom_attributes": [{ "key": "department", "value": "engineering" }],
    ...
    public async Task ListForProjectAsync_BuildsAccessRequestsRoute_AndDeserializesRequesters()
    {
        ...
        AccessRequestsClient repository = new(connection);
        ...
        GitLabCustomAttribute customAttribute = Assert.Single(accessRequest.CustomAttributes ?? []);
        Assert.Equal("department", customAttribute.Key);
        Assert.Equal("engineering", customAttribute.Value);
        Assert.Equal(new Uri("https://gitlab.example/raymond_smith"), accessRequest.WebUrl);
        Assert.Equal(new DateTimeOffset(2012, 10, 22, 14, 13, 35, TimeSpan.Zero), accessRequest.CreatedAt);
    ...
    [Fact]
    public async Task ListForProjectAsync_DeserializesSparseSchemaPermittedRequester()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("[{}]", Encoding.UTF8, "application/json")
        ...
        List<GitLabAccessRequest> requests = new();
        await foreach (GitLabAccessRequest request in repository.ListForProjectAsync(1,
                           TestContext.Current.CancellationToken))
        {
            requests.Add(request);
        }

        GitLabAccessRequest requester = Assert.Single(requests);
        Assert.Null(requester.Id);
        Assert.Null(requester.Username);
        Assert.Null(requester.Name);
        Assert.Null(requester.WebUrl);
    }
