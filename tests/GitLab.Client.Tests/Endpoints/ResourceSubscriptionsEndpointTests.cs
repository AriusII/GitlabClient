...
public sealed class ResourceSubscriptionsEndpointTests
{
    ...
    private const string UnsubscribedGroupLabelJson = """
                                                      {
                                                        ...
                                                        "description_html": "<p>Group-wide planning label</p>",
                                                        "color": "#8E44AD",
                                                        "text_color": "#FFFFFF",
                                                        "archived": false,
                                                        "open_issues_count": 3,
    ...
    public async Task SubscribeToIssueAsync_PostsEmptyBodyToSubscribeRoute_AndDeserializesIssue()
    {
        ...
        ResourceSubscriptionsClient repository = new(connection);
    ...
    public async Task SubscribeToGroupLabelAsync_BuildsGroupRoute_AndEncodesNamespacedGroupPath()
    {
        ...
        GitLabGroupLabel label = await repository.SubscribeToGroupLabelAsync("gitlab-org/subgroup", "group::planning",
            TestContext.Current.CancellationToken);
        ...
        Assert.Equal("<p>Group-wide planning label</p>", label.DescriptionHtml);
        Assert.False(label.Archived);
    ...
    public async Task UnsubscribeFromGroupLabelAsync_BuildsGroupUnsubscribeRoute()
    {
        ...
        GitLabGroupLabel label = await repository.UnsubscribeFromGroupLabelAsync(42, "group::planning",
            TestContext.Current.CancellationToken);
