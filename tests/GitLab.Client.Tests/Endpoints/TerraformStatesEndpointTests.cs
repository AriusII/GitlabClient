...
public sealed class TerraformStatesEndpointTests
{
    /// <summary>
    ...
    public async Task DownloadAsync_EscapesTheStateName_AndStreamsTheOpaqueDocument()
    {
        ...
        TerraformStatesClient repository = new(connection);
    ...
    [Fact]
    public async Task ListProtectionRulesAsync_FollowsThePaginationLinkHeader()
    {
        int callCount = 0;
        ...
        using StubHttpMessageHandler handler = new(request =>
        {
            ...
            if (request.RequestUri!.AbsoluteUri ==
                "https://gitlab.example/api/v4/projects/7/terraform/state_protection_rules")
            {
                ...
                response.Headers.TryAddWithoutValidation("Link",
                    "<https://gitlab.example/api/v4/projects/7/terraform/state_protection_rules?page=2>; rel=\"next\"");
                return response;
            }
