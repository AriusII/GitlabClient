...
/// <summary>
///     Covers the twelve part-C typed per-slug setters added to <see cref="IntegrationsClient" />:
...
///     Mattermost. Each test proves the same three things the base <c>IntegrationsEndpointTests</c>
...
public sealed class IntegrationsRepositoryCTests
{
    ...
    [SuppressMessage(
        ...
        Justification =
            ...
            "exit.")]
    private static (IntegrationsClient Repository, StubHttpMessageHandler Handler, Func<string?> SentBody)
        CreateRepository()
    {
        ...
        return (new IntegrationsClient(connection), handler, () => sentBody);
    }

    [Fact]
    public async Task SetGoogleCloudPlatformArtifactRegistryAsync_PutsToTheModernIntegrationsRoute()
    {
        (IntegrationsClient repository, StubHttpMessageHandler handler, Func<string?> sentBody) =
            CreateRepository();
