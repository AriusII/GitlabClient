...
public sealed class CiLintEndpointTests
{
    ...
    public async Task ValidateProjectConfigurationAsync_BuildsCiLintRoute_WithEveryOption_AndDeserializesTheResult()
    {
        ...
        CiLintClient repository = new(connection);
        ...
        Assert.Equal(
            ...
            include.Raw);
        Assert.Equal("test", include.Extra?.GetProperty("job_name").GetString());
    ...
    [Fact]
    public async Task ValidateAsync_WithExplicitNullContent_PreservesTheRequiredNullableMember()
    {
        const string Json = """{ "valid": false, "errors": ["content is required"] }""";
        ...
        await repository.ValidateAsync(1, new ValidateCiConfigurationRequest { Content = null },
            TestContext.Current.CancellationToken);

        Assert.Equal("{\"content\":null}", sentBody);
    }
