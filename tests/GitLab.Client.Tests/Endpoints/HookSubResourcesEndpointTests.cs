...
/// </summary>
public sealed class HookSubResourcesEndpointTests
{
    ...
    [InlineData(GitLabWebhookTestTrigger.TagPushEvents, "tag_push_events")]
    ...
    [InlineData(GitLabWebhookTestTrigger.WikiPageEvents, "wiki_page_events")]
    ...
    [InlineData(GitLabWebhookTestTrigger.ReleasesEvents, "releases_events")]
    [InlineData(GitLabWebhookTestTrigger.EmojiEvents, "emoji_events")]
    [InlineData(GitLabWebhookTestTrigger.ResourceAccessTokenEvents, "resource_access_token_events")]
    [InlineData(GitLabWebhookTestTrigger.ResourceDeployTokenEvents, "resource_deploy_token_events")]
    [InlineData(GitLabWebhookTestTrigger.VulnerabilityEvents, "vulnerability_events")]
    ...
    public async Task ListEventsAsync_ProjectsTheStatusFilter_AsACommaJoinedQuery()
    {
        ...
        HookEventListOptions options = new() { Status = ["500", "server_failure"], Page = 2, PerPage = 20 };
        ...
        Assert.Equal(
            "https://gitlab.example/api/v4/groups/9/hooks/4/events?status=500,server_failure&page=2&per_page=20",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
    }
    ...
    public async Task DeleteUrlVariableAsync_PercentEncodesTheKey_OnEveryHookSurface()
    {
        ...
        await new ProjectHooksClient(connection).DeleteUrlVariableAsync(42, 7, "token/1",
            TestContext.Current.CancellationToken);
        Assert.Equal(HttpMethod.Delete, handler.LastRequest?.Method);
        Assert.Equal("https://gitlab.example/api/v4/projects/42/hooks/7/url_variables/token%2F1",
            handler.LastRequest?.RequestUri?.AbsoluteUri);

        await new GroupHooksClient(connection).DeleteUrlVariableAsync(9, 4, "token/1",
            TestContext.Current.CancellationToken);
        Assert.Equal("https://gitlab.example/api/v4/groups/9/hooks/4/url_variables/token%2F1",
            handler.LastRequest?.RequestUri?.AbsoluteUri);

        await new SystemHooksClient(connection).DeleteUrlVariableAsync(3, "token/1",
            TestContext.Current.CancellationToken);
    ...
    public async Task DeleteCustomHeaderAsync_PercentEncodesTheKey_OnEveryHookSurface()
    {
        ...
        await new ProjectHooksClient(connection).DeleteCustomHeaderAsync(42, 7, "X-Team Header",
            TestContext.Current.CancellationToken);
        Assert.Equal("https://gitlab.example/api/v4/projects/42/hooks/7/custom_headers/X-Team%20Header",
            handler.LastRequest?.RequestUri?.AbsoluteUri);

        await new GroupHooksClient(connection).DeleteCustomHeaderAsync("gitlab-org/subgroup", 4, "X-Team Header",
            TestContext.Current.CancellationToken);
        Assert.Equal(
            "https://gitlab.example/api/v4/groups/gitlab-org%2Fsubgroup/hooks/4/custom_headers/X-Team%20Header",
            handler.LastRequest?.RequestUri?.AbsoluteUri);

        await new SystemHooksClient(connection).DeleteCustomHeaderAsync(3, "X-Team Header",
            TestContext.Current.CancellationToken);
    ...
    public async Task UpdateUrlVariableAsync_PutsTheValue_WithNoResponseBodyExpected_OnEveryHookSurface()
    {
        ...
        await new ProjectHooksClient(connection).UpdateUrlVariableAsync(42, 7, "token/1",
            new UpdateProjectHookUrlVariableRequest { Value = "s3cr3t" }, TestContext.Current.CancellationToken);
        Assert.Equal(HttpMethod.Put, handler.LastRequest?.Method);
        Assert.Equal("https://gitlab.example/api/v4/projects/42/hooks/7/url_variables/token%2F1",
            handler.LastRequest?.RequestUri?.AbsoluteUri);

        await new GroupHooksClient(connection).UpdateUrlVariableAsync(9, 4, "token/1",
    ...
    public async Task UpdateCustomHeaderAsync_PutsTheValue_WithNoResponseBodyExpected_OnEveryHookSurface()
    {
        ...
        await new ProjectHooksClient(connection).UpdateCustomHeaderAsync(42, 7, "X-Team Header",
        ...
        await new GroupHooksClient(connection).UpdateCustomHeaderAsync("gitlab-org/subgroup", 4, "X-Team Header",
