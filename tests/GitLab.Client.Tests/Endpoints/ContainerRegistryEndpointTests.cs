...
public sealed class ContainerRegistryEndpointTests
{
    ...
    public async Task IngestEventsAsync_PostsToTheEventsRoute_WithTheRawPayload()
    {
        ...
        ContainerRegistryClient repository = new(connection);
    ...
    [Fact]
    public async Task IngestEventsAsync_RejectsAnUndefinedPayload_BeforeSendingARequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK));
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        GitLabApiConnection connection = new(httpClient);
        ContainerRegistryClient client = new(connection);

        ArgumentException exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            client.IngestEventsAsync(default, TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task DeleteTagsAsync_RejectsMissingDeletionMatcher_BeforeSendingARequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.NoContent));
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        GitLabApiConnection connection = new(httpClient);
        ...
        ArgumentException nullOptionsException = await Assert.ThrowsAsync<ArgumentException>(() =>
            client.DeleteTagsAsync(9, 1, cancellationToken: TestContext.Current.CancellationToken));
        ArgumentException filterOnlyException = await Assert.ThrowsAsync<ArgumentException>(() =>
            client.DeleteTagsAsync(
                9,
                1,
                new DeleteRegistryRepositoryTagsOptions { OlderThan = "1month" },
                TestContext.Current.CancellationToken));

        Assert.Equal("options", nullOptionsException.ParamName);
        Assert.Equal("options", filterOnlyException.ParamName);
        Assert.Null(handler.LastRequest);
    }

    [Fact]
    public async Task DeleteTagsAsync_RejectsNegativeKeepN_BeforeSendingARequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.NoContent));
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        GitLabApiConnection connection = new(httpClient);
        ...
        ArgumentOutOfRangeException exception = await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.DeleteTagsAsync(
                9,
                1,
                new DeleteRegistryRepositoryTagsOptions { NameRegexDelete = ".*", KeepN = -1 },
                TestContext.Current.CancellationToken));
    ...
    [Fact]
    public async Task RepositoryOperations_RejectNonPositiveRepositoryIds_BeforeSendingARequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK));
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        GitLabApiConnection connection = new(httpClient);
        ContainerRegistryClient client = new(connection);
        DeleteRegistryRepositoryTagsOptions deletionOptions = new() { NameRegexDelete = ".*" };

        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.DeleteRepositoryAsync(9, 0, TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.DeleteTagsAsync(9, 0, deletionOptions, TestContext.Current.CancellationToken));
        Assert.Throws<ArgumentOutOfRangeException>(() =>
            client.ListTagsAsync(9, 0, cancellationToken: TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.DeleteTagAsync(9, 0, "latest", TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.GetTagAsync(9, 0, "latest", TestContext.Current.CancellationToken));
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            client.GetAsync(0, cancellationToken: TestContext.Current.CancellationToken));
