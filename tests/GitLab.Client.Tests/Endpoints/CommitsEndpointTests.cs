...
public sealed class CommitsEndpointTests
{
    [Fact]
    public async Task GetAsync_BuildsExpectedRoute_AndDeserializesCommit()
    {
        ...
        CommitsClient repository = new(connection);
    ...
    public async Task ListAsync_BuildsExpectedRouteWithQuery_AndDeserializesCommits()
    {
        ...
        Assert.Equal(
            "https://gitlab.example/api/v4/projects/7/repository/commits?all=true&author=Jane%20Doe"
            + "&first_parent=true&follow=false&order=topo&path=src%2FClient.cs&ref_name=main&trailers=true"
            + "&with_stats=true&per_page=20",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
    ...
    [Fact]
    public async Task CreateAsync_PostsJsonActionArray_WithEnumWireValuesAndAllowEmpty_AndDeserializesDetailedCommit()
    {
        const string Json = """
                            {
        ...
        Assert.Contains("\"allow_empty\":false", sentBody, StringComparison.Ordinal);
    ...
    public async Task CreateAsync_WithFile_PostsTheRequiredFilePart()
    {
        ...
        GitLabApiConnection connection = new(httpClient);
        CommitsClient repository = new(connection);
        using MemoryStream content = new(Encoding.UTF8.GetBytes("binary commit content"));
        ...
        GitLabCommit commit = await repository.CreateAsync(42, file, TestContext.Current.CancellationToken);
        ...
        Assert.Contains("commit.patch", sentBody, StringComparison.Ordinal);
        Assert.Contains("binary commit content", sentBody, StringComparison.Ordinal);
        Assert.True(content.CanRead);
        Assert.Equal("Binary commit", commit.Title);
    }

    [Fact]
    public async Task CherryPickAsync_EscapesSlashBearingRef_AndPostsJsonBody()
    {
        const string Json = """
                            {
        ...
        await repository.CherryPickAsync(
            42,
            "release/1.0",
            new CherryPickCommitRequest { Branch = "main", DryRun = false, Message = "Backport" },
            TestContext.Current.CancellationToken);
        ...
        Assert.Contains("\"message\":\"Backport\"", sentBody, StringComparison.Ordinal);
    }

    [Fact]
    public async Task CherryPickWithResultAsync_PostsJson_AndDeserializesTheSuccessfulDryRunPayload()
    {
        const string Json = """{ "dry_run": "success" }""";
        ...
        GitLabCommitOperationResult result = await repository.CherryPickWithResultAsync(
            42,
            "release/1.0",
            new CherryPickCommitRequest { Branch = "main", DryRun = true },
            TestContext.Current.CancellationToken);
        ...
        Assert.Equal("success", result.DryRun);
        Assert.Null(result.Commit);
    }

    [Fact]
    public async Task RevertAsync_BuildsRevertRoute_AndPostsJsonBody()
    {
        const string Json = """
                            {
                              "id": "8b090c1b79a14f2bd9e8a738f717824ff53aebad",
    ...
    [Fact]
    public async Task RevertWithResultAsync_PostsJson_AndDeserializesTheSuccessfulDryRunPayload()
    {
        const string Json = """{ "dry_run": "success" }""";
        ...
        GitLabCommitOperationResult result = await repository.RevertWithResultAsync(
            42,
            "deadbeef",
            new RevertCommitRequest { Branch = "main", DryRun = true },
            TestContext.Current.CancellationToken);

        Assert.Equal("https://gitlab.example/api/v4/projects/42/repository/commits/deadbeef/revert",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
        Assert.Equal("application/json", sentMediaType);
        Assert.True(result.IsDryRun);
        Assert.Equal("success", result.DryRun);
    ...
    [Fact]
    public async Task CreateCommentAsync_PostsJsonWithLineTypeWireValue()
    {
        const string Json = """
                            {
                              "note": "nice",
        ...
        Assert.Contains("\"note\":\"nice\"", sentBody, StringComparison.Ordinal);
