...
public sealed class BulkImportsEndpointTests
{
    ...
    public async Task ListAsync_BuildsTheMigrationRoute_AndDeserializesTheMigration()
    {
        ...
        BulkImportsClient repository = new(connection);
    ...
    [Fact]
    public async Task CreateAsync_PostsTheSourceConfigurationAndEntitiesAsUrlEncodedFormFields()
    {
        ...
        Assert.Contains("configuration%5Burl%5D=https%3A%2F%2Fsource.gitlab.example", sentBody,
            StringComparison.Ordinal);
        Assert.Contains("configuration%5Baccess_token%5D=glpat-source-token", sentBody, StringComparison.Ordinal);

        // The request vocabulary is "group_entity"/"project_entity", not the response's "group"/"project".
        Assert.Contains("entities%5B0%5D%5Bsource_type%5D=group_entity", sentBody, StringComparison.Ordinal);
        Assert.Contains("entities%5B0%5D%5Bsource_full_path%5D=source%2Ffull%2Fpath", sentBody,
            StringComparison.Ordinal);
        Assert.Contains("entities%5B0%5D%5Bdestination_namespace%5D=destination%2Fnamespace", sentBody,
            StringComparison.Ordinal);
        Assert.Contains("entities%5B0%5D%5Bdestination_slug%5D=destination-slug", sentBody,
            StringComparison.Ordinal);
        Assert.Contains("entities%5B0%5D%5Bmigrate_projects%5D=false", sentBody, StringComparison.Ordinal);
        Assert.Contains("entities%5B0%5D%5Bmigrate_memberships%5D=true", sentBody, StringComparison.Ordinal);
    ...
    [Fact]
    public async Task ListEntitiesAsync_ProjectsTheInstanceWideSortParameter()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK)
        {
            Content = new StringContent("[]", Encoding.UTF8, "application/json")
        ...
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        BulkImportsClient repository = new(new GitLabApiConnection(httpClient));

        await foreach (GitLabBulkImportEntity _ in repository.ListEntitiesAsync(
                           new BulkImportEntityListOptions { Sort = GitLabBulkImportSort.Asc },
                           TestContext.Current.CancellationToken))
        {
            Assert.Fail("The stubbed response is an empty page.");
        }

        Assert.Equal("https://gitlab.example/api/v4/bulk_imports/entities?sort=asc",
            handler.LastRequest?.RequestUri?.AbsoluteUri);
    }
    ...
    public async Task ListEntitiesForImportAsync_ScopesToOneMigration_AndFiltersByStatus()
    {
        ...
        BulkImportEntityForImportListOptions options = new()
        {
            Status = GitLabBulkImportStatus.Failed, Page = 3, PerPage = 20
        };
    ...
    [Fact]
    public void BulkImportEntityForImportListOptions_ExposesOnlyThePerImportQueryVocabulary()
    {
        Assert.Null(typeof(BulkImportEntityForImportListOptions)
            .GetProperty(nameof(BulkImportEntityListOptions.Sort)));
    }
