...
/// </summary>
public sealed class ProjectImportEndpointTests
{
    ...
    public static TheoryData<object, string[], string> SecretBearingRequests =>
        new()
        {
            {
                new ImportProjectFromGitRequest
                {
                    ImportUrl =
                        new Uri("https://git-user:git-password@example.invalid/repository.git?token=url-secret"),
                    ImportUsername = "git-user",
                    ImportPassword = "git-password"
                },
                ["url-secret", "git-user", "git-password"], "[redacted]"
            },
            {
                new ImportProjectFromGitHubRequest
                {
                    ...
                    GithubHostname = new Uri("https://github.example.invalid")
                },
                ["github-token", "github.example.invalid"], "[redacted]"
            },
            {
                new ImportProjectFromBitbucketRequest
                {
                    BitbucketEmail = "user@example.invalid",
                    BitbucketApiToken = "bitbucket-token",
                    RepoPath = "workspace/repository",
                    TargetNamespace = "team"
                },
                ["user@example.invalid", "bitbucket-token"], "[redacted]"
            },
            {
                new ImportProjectFromBitbucketServerRequest
                {
                    BitbucketServerUrl = new Uri("https://bitbucket.example.invalid"),
                    BitbucketServerUsername = "service-account",
                    PersonalAccessToken = "server-token",
                    BitbucketServerProject = "TEAM",
                    BitbucketServerRepo = "repository"
                },
                ["bitbucket.example.invalid", "service-account", "server-token"], "[redacted]"
            },
            {
                new ImportProjectFromS3Request
                {
                    Region = "eu-west-1",
                    BucketName = "backups",
                    FileKey = "exports/project.tar.gz",
                    AccessKeyId = "aws-access-key",
                    SecretAccessKey = "aws-secret-key",
                    Path = "restored"
                },
                ["aws-access-key", "aws-secret-key"], "[redacted]"
            },
            {
                new ImportProjectFromRemoteArchiveRequest
                {
                    Url = new Uri("https://storage.example.invalid/export.tar.gz?X-Amz-Signature=remote-secret"),
                    Path = "restored"
                },
                ["storage.example.invalid", "remote-secret"], "[redacted]"
            },
            {
                new ImportProjectArchiveRequest
                {
                    Path = "restored",
                    OverrideParams =
                        new Dictionary<string, string>(StringComparer.Ordinal)
                        {
                            ["import_url"] = "https://user:archive-secret@example.invalid/repository.git"
                        }
                },
                ["archive-secret"], "[present]"
            },
            {
                new GitLabProjectExportUpload
                {
                    Url = new Uri("https://storage.example.invalid/export.tar.gz?X-Amz-Signature=upload-secret")
                },
                ["storage.example.invalid", "upload-secret"], "[redacted]"
            }
        };
    ...
    public async Task GetExportStatusAsync_BuildsExportRoute_AndDeserializesTheStatusVocabulary()
    {
        ...
        ProjectImportClient repository = new(new GitLabApiConnection(httpClient));
    ...
    [Theory]
    [InlineData("none", GitLabProjectExportState.None)]
    [InlineData("regeneration_in_progress", GitLabProjectExportState.RegenerationInProgress)]
    public async Task GetExportStatusAsync_DeserializesAdditionalDocumentedStateValues(string wireValue,
        GitLabProjectExportState expectedState)
    {
        string response = ExportStatusJson.Replace("\"finished\"", $"\"{wireValue}\"", StringComparison.Ordinal);
        using StubHttpMessageHandler handler = Json(response);
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        ProjectImportClient repository = new(new GitLabApiConnection(httpClient));
        ...
        Assert.Equal(expectedState, status.ExportStatus);
    }
    ...
    [Fact]
    public async Task GetRelationImportStatusAsync_ReadsTheDocumentedTrackerArray()
    {
        ...
        using StubHttpMessageHandler handler = Json(ResponseJson);
        using HttpClient httpClient = new(handler) { BaseAddress = BaseAddress };
        ProjectImportClient repository = new(new GitLabApiConnection(httpClient));

        IReadOnlyList<GitLabProjectRelationImport> trackers =
        ...
        GitLabProjectRelationImport tracker = Assert.Single(trackers);
        Assert.Equal(42, tracker.Id);
        Assert.Equal("gitlab-org/gitlab-test", tracker.ProjectPath);
        Assert.Equal("issues", tracker.Relation);
        Assert.Equal("finished", tracker.Status);
    }
    ...
    public async Task ImportArchiveAsync_PostsAMultipartBodyWithBracketedOverrideParams()
    {
        ...
        Assert.Contains("name=namespace", body, StringComparison.Ordinal);
        Assert.Contains("gitlab-org/sub", body, StringComparison.Ordinal);
    ...
    [Fact]
    public async Task ImportFromRemoteArchiveAsync_PostsTheRemoteImportMultipartFields()
    {
        using CapturingHttpMessageHandler handler = new(() => new HttpResponseMessage(HttpStatusCode.Created)
        {
            Content = new StringContent(ImportStatusJson, Encoding.UTF8, "application/json")
        ...
        Assert.Contains("https://example.invalid/export.tar.gz", body, StringComparison.Ordinal);
        Assert.Contains("name=namespace", body, StringComparison.Ordinal);
        Assert.Contains("legacy-team", body, StringComparison.Ordinal);
        Assert.Contains("name=overwrite", body, StringComparison.Ordinal);
        Assert.Contains("false", body, StringComparison.Ordinal);
        Assert.Contains("name=override_params[description]", body, StringComparison.Ordinal);
        Assert.Contains("from remote", body, StringComparison.Ordinal);
    }

    [Fact]
    public async Task ImportFromS3Async_PostsTheS3MultipartFieldsWithItsCredentials()
    {
        using CapturingHttpMessageHandler handler = new(() => new HttpResponseMessage(HttpStatusCode.Created)
        {
            Content = new StringContent(ImportStatusJson, Encoding.UTF8, "application/json")
        ...
        Assert.StartsWith("multipart/form-data", handler.RequestContentType, StringComparison.Ordinal);
        string body = handler.RequestBody.Replace("\"", string.Empty, StringComparison.Ordinal);
        Assert.Contains("name=region", body, StringComparison.Ordinal);
        Assert.Contains("eu-west-1", body, StringComparison.Ordinal);
        Assert.Contains("name=bucket_name", body, StringComparison.Ordinal);
        Assert.Contains("backups", body, StringComparison.Ordinal);
        Assert.Contains("name=file_key", body, StringComparison.Ordinal);
        Assert.Contains("exports/gitlab-test.tar.gz", body, StringComparison.Ordinal);
        Assert.Contains("name=access_key_id", body, StringComparison.Ordinal);
        Assert.Contains("AKIAEXAMPLE", body, StringComparison.Ordinal);
        Assert.Contains("name=secret_access_key", body, StringComparison.Ordinal);
        Assert.Contains("s3cret", body, StringComparison.Ordinal);
        Assert.Contains("name=namespace", body, StringComparison.Ordinal);
        Assert.Contains("legacy-team", body, StringComparison.Ordinal);
    }
    ...
    [Theory]
    [MemberData(nameof(SecretBearingRequests))]
    public void SecretBearingImportRequests_ToString_RedactsCredentialsAndPresignedUrls(object request,
        string[] secrets, string redactionMarker)
    {
        ArgumentNullException.ThrowIfNull(request);
        ArgumentNullException.ThrowIfNull(secrets);
        ArgumentNullException.ThrowIfNull(redactionMarker);

        string text = request.ToString() ?? string.Empty;
        ...
        Assert.Contains(redactionMarker, text, StringComparison.Ordinal);
    }
