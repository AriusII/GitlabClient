...
public sealed class NotificationSettingsEndpointTests
{
    ...
    public async Task GetGlobalAsync_BuildsInstanceRoute_AndDeserializesCustomFlags()
    {
        ...
        NotificationSettingsClient repository = new(connection);
        ...
        Assert.Equal("all", settings.Events);
    ...
    [Fact]
    public async Task GetGlobalAsync_DeserializesSchemaPermittedPayloadWithoutLevel()
    {
        const string Json = """{ "events": "participating" }""";
        ...
        GitLabNotificationSettings settings = await repository.GetGlobalAsync(TestContext.Current.CancellationToken);

        Assert.Null(settings.Level);
        Assert.Equal("participating", settings.Events);
