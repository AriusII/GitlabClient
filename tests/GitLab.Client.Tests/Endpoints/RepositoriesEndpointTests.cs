...
public sealed class RepositoriesEndpointTests
{
    ...
    public async Task ListTreeAsync_BuildsTreeRoute_WithQueryOptions_AndDeserializesEntries()
    {
        ...
        RepositoriesClient repository = new(connection);
    ...
    [Fact]
    public async Task GetMergeBaseAsync_WithABlankRef_ThrowsBeforeAnyRequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK));
        ...
        ArgumentException exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            repository.GetMergeBaseAsync(1, ["main", " "], TestContext.Current.CancellationToken));

        Assert.Equal("refs[1]", exception.ParamName);
        Assert.Null(handler.LastRequest);
    }

    [Fact]
    public async Task CompareAsync_WithABlankRequiredRef_ThrowsBeforeAnyRequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK));
        ...
        ArgumentException exception = await Assert.ThrowsAsync<ArgumentException>(() =>
            repository.CompareAsync(1, "main", " ", cancellationToken: TestContext.Current.CancellationToken));

        Assert.Equal("toRef", exception.ParamName);
        Assert.Null(handler.LastRequest);
    }
    ...
    [Fact]
    public async Task GetBlobsAsync_WithoutARef_UsesGitLabsDefaultBranch()
    {
        const string Json =
            """[ { "path": "README.md", "size": 25, "truncated": false, "encoding": "base64", "content": "IyBSRUFETUUK" } ]""";
        ...
        IReadOnlyList<GitLabBatchBlob> blobs = await repository.GetBlobsAsync(42,
            new BlobBatchRequest { Files = [new BlobBatchFile { Path = "README.md" }] },
            TestContext.Current.CancellationToken);

        Assert.Single(blobs);
        Assert.Contains("\"path\":\"README.md\"", sentBody, StringComparison.Ordinal);
        Assert.DoesNotContain("\"ref\"", sentBody, StringComparison.Ordinal);
    }

    [Fact]
    public async Task GetBlobsAsync_WithMoreThanTwentyFiles_ThrowsBeforeAnyRequest()
    {
        using StubHttpMessageHandler handler = new(_ => new HttpResponseMessage(HttpStatusCode.OK));
        ...
        GitLabApiConnection connection = new(httpClient);
        RepositoriesClient repository = new(connection);
        List<BlobBatchFile> files = Enumerable.Range(0, 21)
            .Select(index => new BlobBatchFile { Path = $"file-{index}.txt" })
            .ToList();

        ArgumentOutOfRangeException exception = await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            repository.GetBlobsAsync(42, new BlobBatchRequest { Files = files },
                TestContext.Current.CancellationToken));
    ...
    public async Task GetHealthAsync_SendsGenerate_AndDeserializesTheNestedStatistics()
    {
        const string Json = """
                            {
                              ...
                              "alternates": { "pool": "/var/opt/gitlab/git-data/repositories/@pools/abc.git" },
        ...
        Assert.True(health.Alternates.HasValue);
        Assert.Equal(JsonValueKind.Object, health.Alternates.Value.ValueKind);
        Assert.Equal("/var/opt/gitlab/git-data/repositories/@pools/abc.git",
            health.Alternates.Value.GetProperty("pool").GetString());
