...
public sealed record GitLabIssue
{
    ...
    /// <summary>The endpoint routes GitLab includes under its <c>_links</c> member.</summary>
    [JsonPropertyName("_links")]
    public GitLabIssueLinks? Links { get; init; }
    ...
    /// <summary>
    ///     The internal ID of the containing epic. The value is a string in GitLab's response schema, even
    ///     when the epic's database ID is numeric.
    /// </summary>
    public string? EpicIid { get; init; }

    /// <summary>
    ///     The reduced epic projection GitLab embeds in issue responses. It is absent when the issue is not
