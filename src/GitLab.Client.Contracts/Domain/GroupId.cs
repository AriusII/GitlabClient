...
/// <summary>
///     Identifies a GitLab group either by its numeric ID or by its "full path", exactly
...
public readonly record struct GroupId
{
    ...
    /// <summary>Creates a group identifier from an unencoded full path.</summary>
    /// <param name="fullPath">
    ///     The raw full path, not an already percent-encoded route segment. An already encoded value is intentionally
    ///     encoded again by <see cref="ToRouteValue" />; for example, <c>%2F</c> becomes <c>%252F</c>.
    /// </param>
