...
public class GitLabApiException : Exception
{
    ...
    public override string ToString()
    {
        ...
        builder.Append(' ');
        builder.Append(RequestUri is { } requestUri ? GetDiagnosticUri(requestUri) : "(unknown uri)");
        builder.Append(" -> ");
    ...
    private static string GetDiagnosticUri(Uri requestUri)
    {
        // Query strings routinely carry search terms and can accidentally contain a private_token or an
        // access_token. Exception ToString() often lands in application logs, so retain endpoint context
        // while never emitting query, fragment, or URI user-info credentials.
