...
internal sealed class IssuesRepository(IGitLabApiConnection connection) : IIssuesRepository
{
    ...
    public Task DeleteMetricImageAsync(ProjectId projectId, long issueIid, long metricImageId,
        CancellationToken cancellationToken = default)
    {
        return connection.DeleteAsync(
            ProjectIssue(projectId, issueIid).Literal("metric_images").Segment(metricImageId).Build(),
