...
/// <summary>
///     Wraps the GitLab "Job artifacts" API area (<c>/projects/:id/jobs/:job_id/artifacts</c>,
///     <c>/projects/:id/jobs/artifacts/:ref_name</c> and <c>/projects/:id/artifacts</c>).
///     <para>
///         Four of the eight operations return a raw, non-JSON body. Each hands back a
///         <see cref="GitLabFileResponse" /> that owns the HTTP response, the streamed body and the
///         per-operation cancellation source, so every one of them MUST be disposed by the caller -
///         <c>await using</c> - or the pooled connection is never returned.
///     </para>
/// </summary>
public interface IJobArtifactsClient
{
    /// <summary>
