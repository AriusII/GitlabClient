...
/// <summary>
///     GitLab throttled the request (HTTP 429). Wait <see cref="RetryAfter" /> and retry; the request itself was
///     fine. <see cref="GitLabApiException.IsTransient" /> is true.
/// </summary>
public sealed class GitLabRateLimitExceededException : GitLabApiException
{
    ...
    ///     delta-seconds and the HTTP-date forms; null when the header was absent, in which case
    ///     <see cref="GitLabRateLimitSnapshot.ResetsAt" /> is the next best signal. Prefer this over
    ///     <see cref="GitLabRateLimitSnapshot.RetryAfter" />, which only sees the delta-seconds form.
    /// </summary>
