...
internal sealed class MembersClient(IGitLabApiConnection connection) : IMembersClient
{
    public IAsyncEnumerable<GitLabMember> ListAsync(ProjectId projectId,
        CancellationToken cancellationToken = default)
    {
        return ListAsync(projectId, null, cancellationToken);
    }

    public IAsyncEnumerable<GitLabMember> ListAsync(ProjectId projectId, ProjectMemberListOptions? options,
        CancellationToken cancellationToken = default)
    {
        return connection.GetPagedAsync(
            GitLabRouteBuilder.Create("projects").Segment(projectId).Literal("members").QueryFrom(options).Build(),
    ...
    {
        return RemoveAsync(projectId, userId, null, cancellationToken);
    }

    public Task RemoveAsync(ProjectId projectId, long userId, RemoveProjectMemberOptions? options,
        CancellationToken cancellationToken = default)
    {
