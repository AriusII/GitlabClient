...
internal sealed class SnippetsClient(IGitLabApiConnection connection) : ISnippetsClient
{
    ...
    public IAsyncEnumerable<GitLabSnippet> ListForProjectAsync(ProjectId projectId,
        CancellationToken cancellationToken = default)
    {
        return ListForProjectAsync(projectId, default, cancellationToken);
    }
    ...
    {
        return connection.GetPagedAsync(
            GitLabRouteBuilder.Create("projects").Segment(projectId).Literal("snippets")
                .QueryFrom(options)
                .Build(),
            GitLabJsonContext.Default.GitLabSnippetArray,
            cancellationToken);
