...
internal sealed class WikisClient(IGitLabApiConnection connection) : IWikisClient
{
        ...
        string? branch = null, CancellationToken cancellationToken = default)
    {
        ...
        // The Wiki API accepts attachments only under its fixed multipart field name, "file".
        return connection.PostFileAsync(
            GitLabRouteBuilder.Create("projects").Segment(projectId).Literal(WikisSegment).Literal("attachments")
                .Build(),
            file with { FieldName = GitLabFileUpload.DefaultFieldName },
