...
internal sealed class GitLabRetryHandler : DelegatingHandler
{
    ...
    [SuppressMessage(
        ...
        Justification =
            "Every retry clone this handler builds carries only buffer-backed content (ByteArrayContent, " +
            "never a stream - see CanReuseContent), so there is nothing unmanaged to release deterministically; " +
            "disposing it here would break any downstream handler or caller that inspects the sent request " +
            "after this call returns, the way GitLabRetryHandlerTests' fake handler does.")]
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request,
        CancellationToken cancellationToken)
    {
        List<KeyValuePair<string, IEnumerable<string>>> originalHeaders = SnapshotHeaders(request);

        HttpResponseMessage response = await base.SendAsync(request, cancellationToken).ConfigureAwait(false);

        for (int attempt = 1; attempt <= MaxRetryAttempts && IsTransient(response.StatusCode); attempt++)
        {
            ...
            // Deliberately not `using`: the content this handler ever attaches here is buffer-only
            // (ByteArrayContent, never a stream), so there is nothing unmanaged to release deterministically,
            // and eagerly disposing it would break any downstream handler that - like GitLabRateLimitHandler,
            // or a diagnostic/logging handler in a caller's own pipeline - inspects the sent request after
            // this call returns.
            HttpContent? retryContent =
                await CreateRetryContentAsync(request.Content, cancellationToken).ConfigureAwait(false);
            HttpRequestMessage retryRequest = CreateRetryRequest(request, originalHeaders, retryContent);
    ...
    [SuppressMessage(
        "Reliability",
        "CA2000:Dispose objects before losing scope",
        Justification = "See the SendAsync override's identical suppression justification above.")]
    protected override HttpResponseMessage Send(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        for (int attempt = 1; attempt <= MaxRetryAttempts && IsTransient(response.StatusCode); attempt++)
        {
            ...
            // See the SendAsync override for why retryRequest is deliberately not disposed here.
    ...
    private static List<KeyValuePair<string, IEnumerable<string>>> SnapshotHeaders(HttpRequestMessage request)
    {
        List<KeyValuePair<string, IEnumerable<string>>> snapshot = [];

        foreach (KeyValuePair<string, IEnumerable<string>> header in request.Headers)
        {
            snapshot.Add(new KeyValuePair<string, IEnumerable<string>>(header.Key, header.Value.ToArray()));
        }
